...
public class ArticlesSet implements Seriesable {
    ...
    public ArticlesSet() {
        title = "без названия";
        price = Seriesable.MIN_NUM_OF_START_PAGES;
        collection = new String[Seriesable.MIN_NUM_OF_ELS_OF_SERIES];
        durations = new int[collection.length];
    }

    public ArticlesSet(String title, int price, int numOfArticles) {
        this.title = title;
        this.price = price;
        collection = new String[numOfArticles];
        durations = new int[collection.length];
    }
    ...
    private boolean areNumOfStartPagesAndElsEqual(Seriesable anotherSeries) {
        return this.getPrice() == anotherSeries.getPrice() &&
                this.collection.length == anotherSeries.getItemsCount();
    }

    private boolean isElEqual(int index, Seriesable anotherSeries) {
        return collection[index].equals(anotherSeries.getItem(index)) &&
                durations[index] == anotherSeries.getDuration(index);
    }
